...
switch (introstate)
{
	...
	case 2:
		...
		else if (floor(image_index) >= 2 && sprite_index != spr_player_gnomecutscene1)
		{
			with (obj_player)
			{
				if (sprite_index != spr_player_gnomecutscene2 && character == "P")
				{
					sprite_index = spr_player_gnomecutscene2;
					image_index = 0;
				}
				else if (character == "P" && ANIMATION_END)
				{
					image_index = image_number - 1;
				}
			}
	...
	case 4:
		with (obj_player)
		{
			if ((character == "P" && ANIMATION_END) && sprite_index != spr_player_gnomecutscene1)
			{
				image_index = image_number - 1;
			}
		}
		if (ANIMATION_END)
		{
			...
			with (instance_create(x + 42, y + 300, obj_pizzafacep3_prop))
			{
				...
				else if (b[0] == spr_playerN_hurt && obj_player.character == "N")
				{
					usepalette = true;
	...
	case 9:
		...
		if ((obj_player.image_index >= 20 || obj_player.character == "N") && !shot)
		{
			if (obj_player.character == "P")
			{
