...
function scr_vigilante_duel()
{
	...
	var _cheat = obj_player.character == "N" || global.swapmode;
	if (duelphase == 2)
	{
		with (obj_player)
		{
	...
	else if (duelphase < 2)
	{
		with (obj_player)
		{
			if (key_slap2 && character == "P" && !_cheat)
			{
				other.duelbuffer = 0;
				other.duelphase = 2;
	...
	if (duelbuffer > 0)
	{
		...
		switch (duelphase)
		{
			...
			case 1:
				...
				if (obj_player.character == "P" && signy <= -sprite_get_height(signspr))
				{
					duelbuffer = 80;
		...
		if (duelphase == 2 && obj_player.character == "P" && !_cheat)
		{
