...
function draw_enemy(_draw_healthbar, _pal, _color = c_white)
{
	...
	if (visible && object_index != obj_pizzaball && object_index != obj_fakesanta && bbox_in_camera(view_camera[0], 32))
	{
		...
		else if (usepalette && _pal)
		{
			...
			if (!global.swapmode)
			{
				if ((object_index == obj_fakepepboss || object_index == obj_gustavograbbable) && obj_player1.character != "N")
				{
...
function draw_player()
{
	if (instance_exists(obj_swapgusfightball))
	{
		...
		with (obj_swapgusfightball)
		{
			...
			var _info = [[sprite_index, get_char_palette_info("P")], [spr_playerN_fightballratmount, get_char_palette_info("N")]];
	...
	if (character == "N" && state == states.trashstart)
	{
		exit;
	}
	...
	{
		var info = get_char_palette_info("N");
	...
	if (character == "N" && instance_exists(obj_pizzaface_thunderdark))
	{
