function scr_hurtplayer(_player)
{
	...
	with (_player)
	{
		...
		else if (global.noisejetpack == true && (character == "P" || noisepizzapepper))
		{
		}
		...
		if (_hurt)
		{
			...
			else
			{
				with (obj_tv)
				{
					...
					{
						sprite_index = other.character == "P" ? spr_tv_idleN : spr_tv_idle;
						if (other.noisecrusher)
						{
							sprite_index = spr_tv_idleG;
						}
						var _palinfo = other.character == "P" ? get_char_palette_info("N") : get_char_palette_info("P");
				...
				else
				{
					...
					{
						case "N":
							tv_do_expression(spr_tv_exprhurt, false, true);
							break;
						default:
							tv_do_expression(spr_tv_exprhurtN, false, true);
							break;
					}
				}
				switch (character)
						...
						repeat (n)
						{
							...
							if (pos != undefined)
							{
								...
								if (_swap)
								{
									...
									{
										case "N":
											info = get_char_palette_info("N");
										default:
											info = get_char_palette_info("P");
											break;
									}
									...
									hp_sprite = other.character == "P" ? spr_bossfight_noiseHP : spr_bossfight_playerhp;
								}
