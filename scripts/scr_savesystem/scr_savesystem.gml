...
function get_percentage()
{
	...
	for (var i = 0; i < array_length(bosses); i++)
	{
		var boss = bosses[i];
	...
	for (var i = 0; i < array_length(bossranks); i++)
	{
		var boss = bossranks[i];
...
function gamesave_async_load()
{
	with (obj_savesystem)
	{
		if (state == 0)
		{
			...
			buffer_load_async(loadbuff, get_savefile_ini(character), 0, -1);
